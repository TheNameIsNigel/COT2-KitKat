...
RECOVERY_DEV_NAME := Cannibal Open Touch
RECOVERY_DEV_VERSION := v3.0
ifneq ($(BOARD_RECOVERY_DEV_RELTYPE),)
	RECOVERY_DEV_RELTYPE := $(BOARD_RECOVERY_DEV_RELTYPE)
else
	RECOVERY_DEV_RELTYPE := Alpha
endif

RECOVERY_VERSION := $(RECOVERY_DEV_NAME) $(RECOVERY_DEV_VERSION) $(RECOVERY_DEV_RELTYPE)
...
BOARD_RECOVERY_DEFINES := BOARD_HAS_NO_SELECT_BUTTON BOARD_UMS_LUNFILE BOARD_RECOVERY_ALWAYS_WIPES BOARD_RECOVERY_HANDLES_MOUNT RECOVERY_EXTEND_NANDROID_MENU TARGET_USE_CUSTOM_LUN_FILE_PATH TARGET_DEVICE TARGET_RECOVERY_FSTAB
