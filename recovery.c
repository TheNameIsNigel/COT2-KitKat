...
void delayed_reboot() {
  int i;
  for (i = 3; i > 0; i--) {
    ui_print("Rebooting the system in (%d)\n", i);
    sleep(1);
  }
  pass_normal_reboot();
}
...
int run_script_file(void) {
  ...
  if (fp != NULL) {
	...
	if (value[0] != '/') {
	  //relative path given
	  sprintf(full_path, "%s/%s", get_primary_storage_path(), value);
...
main(int argc, char **argv) {
    ...
    ui_print("Rebooting...\n");
    pass_normal_reboot(ANDROID_RB_RESTART, 0, 0);
